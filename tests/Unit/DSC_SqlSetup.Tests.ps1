...
function Invoke-TestCleanup
{
    ...
    Write-Verbose -Message ('Test {1} ran for {0} minutes' -f ([System.TimeSpan]::FromMilliseconds($script:timer.ElapsedMilliseconds)).ToString('hh\:mm\:ss'), $script:dscResourceName) -Verbose
    $script:timer.Stop()
}
